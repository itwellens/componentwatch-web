...
export default async function AIBuildsPage() {
  ...
  return (
    <div className="mx-auto max-w-content px-6 py-12">
      ...
      {/* Hero */}
      <div className="relative rounded-2xl border border-accent/20 bg-gradient-to-br from-accent/8 via-transparent to-transparent p-10 mb-12 overflow-hidden">
        <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_70%_80%_at_0%_0%,rgba(77,142,240,0.08)_0%,transparent_100%)]" />
        <div className="relative">
          <p className="text-[11px] font-semibold uppercase tracking-widest text-accent mb-3">Local AI</p>
          ...
          <p className="text-sm text-ink-muted max-w-2xl leading-relaxed">
            ...
            These compact builds pack high-end GPUs into cases under 20L — always-on and powerful
            enough for serious inference workloads.
          </p>
        </div>
      </div>
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-12">
        {[
          {
            title: 'VRAM first',
            body:  'For 7B models: 8 GB minimum. For 13B+: 16 GB. For 70B quantized: 24 GB+. The RTX 3090/4090 with 24 GB VRAM is the gold standard for local LLM work.',
          },
          {
          ...
          {
            title: 'Power draw',
            body:  "A 4090 pulls 450 W under load. In a small case on a 300 W PSU that's a problem. Check power draw figures against each build's PSU rating before replicating.",
          },
        ].map(item => (
      ...
      <section>
        ...
        {builds.length === 0 ? (
          <div className="rounded-2xl border border-line bg-surface p-16 text-center">
            <p className="text-sm text-ink-muted">No benchmarked builds found — check back as more are added.</p>
          </div>
        ) : (
      ...
      {/* Coming soon */}
      <section className="mt-16 rounded-2xl border border-dashed border-line p-8 text-center">
        <h3 className="text-sm font-semibold text-ink mb-2">Inference benchmarks coming soon</h3>
